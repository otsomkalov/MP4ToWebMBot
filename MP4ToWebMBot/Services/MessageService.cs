...
namespace MP4ToWebMBot.Services
{
    ...
    public class MessageService : IMessageService
    {
        ...
        private static readonly Regex Mp4Regex = new Regex("https?[^ ]*.mp4");
        ...
        {
            if (message?.Document?.FileName?.EndsWith(".mp4", StringComparison.InvariantCultureIgnoreCase) == true)
            {
            ...
            if (!string.IsNullOrEmpty(message?.Text))
            {
                ...
                {
                    var matches = Mp4Regex.Matches(message.Text);
            ...
            if (!string.IsNullOrEmpty(message?.Caption))
            {
                if (message.Caption.Contains("!nsfw", StringComparison.InvariantCultureIgnoreCase) != true)
                {
                    var matches = Mp4Regex.Matches(message.Caption);
        ...
        private async void HandleLinkAsync(Message receivedMessage, string link)
        {
            try
            {
                var inputFileName = $"{Path.GetTempPath()}{Guid.NewGuid()}.mp4";
